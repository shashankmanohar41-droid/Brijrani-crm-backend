...
export interface CalculationInput {
  ...
  quantity: number;
  baseRate: number; // Base purchase rate per MT/Qtl
  calculationMethod: 'Discount' | 'Pro-Rata' | 'Both';
  rebateType: 'Standard Rebate' | 'Single Rebate' | 'Double Rebate' | 'All' | 'All Types';
  discountRate?: number; // % e.g. 0.05 or flat amount
  discountType?: 'PERCENT' | 'FLAT';
  qualityParameters?: QualityParameterInput[];
  applicableRules?: any[];
...
export interface QualityCalculationOutput {
  baseValue: number;
  parameterCalculations: ParameterCalculationResult[];
  totalRebate: number; // Deduction per MT/Qtl
...
export function calculateQualityRebate(input: CalculationInput): QualityCalculationOutput {
  const {
    quantity = 0,
    baseRate = 0,
  ...
  const parameterCalculations: ParameterCalculationResult[] = [];
  const ruleIdsUsed: string[] = [];
  const breakdownDetails: string[] = [];

  let totalRebate = 0; // per unit
  ...
  if (calculationMethod === 'Discount') {
    ...
    for (const param of qualityParameters) {
      parameterCalculations.push({
        ...
        formulaDescription: `Calculated via Global Discount Method (₹${discountAmountPerUnit}/unit)`,
        status: 'PASS'
      });
    }

    const summaryText = `Discount calculation applied: Base Rate ₹${validBaseRate.toLocaleString('en-IN')} - Discount ₹${totalRebate} = Final Rate ₹${finalRate.toLocaleString('en-IN')}/unit. Total = ₹${finalValue.toLocaleString('en-IN')}`;
  ...
  let paramsToProcess: QualityParameterInput[] = [];
  if (rebateType === 'Single Rebate') {
    ...
    // Standard Rebate / All Types: all parameters
    paramsToProcess = qualityParameters;
  }
      ...
      if (matchingRule && Array.isArray(matchingRule.slabs) && matchingRule.slabs.length > 0) {
        ...
        if (matchingSlab) {
          ...
          if (matchingSlab.rateType === 'Fixed Amount') {
            ...
            formulaDesc = `Deviation (${rawDeviation.toFixed(2)}${unit}) matches Slab [${matchingSlab.minDeviation}-${matchingSlab.maxDeviation}${unit}] @ ${slabRate}% of Base = ₹${rebatePerUnit}/unit`;
          } else {
            rebatePerUnit = round2(netDeviation * slabRate);
            formulaDesc = `Deviation (${rawDeviation.toFixed(2)}${unit}) matches Slab [${matchingSlab.minDeviation}-${matchingSlab.maxDeviation}${unit}]: Net Dev (${netDeviation.toFixed(2)}${unit}) × ₹${slabRate} = ₹${rebatePerUnit}/unit`;
          }
        } else {
        ...
        formulaDesc = `Net Dev (${netDeviation.toFixed(2)}${unit}) × ${pctRate}% of Base Rate (₹${validBaseRate}) = ₹${rebatePerUnit}/unit`;
      } else if (matchingRule && matchingRule.rebateBasis === 'Flat Rate per MT') {
        rebatePerUnit = round2(Number(matchingRule.rebateRate) || 0);
        formulaDesc = `Flat Rebate configured in Master = ₹${rebatePerUnit}/unit`;
      } else {
